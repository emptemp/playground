...
def spotisearch(content):
	...
	for band in content:
		...
		if albums:
			...
			album_name = albums[0]['name'].encode("utf-8")
			#print " ",
			#print albums[0]['artists'][0]['name']
			artist_name = albums[0]['artists'][0]['name'].encode("utf-8")
			album_id = albums[0]['uri'].replace("spotify:album:","")
			#uartist_name = unicode(artist_name, 'utf-8')
			#ualbum_name = unicode(album_name, 'utf-8')
			print album_id
